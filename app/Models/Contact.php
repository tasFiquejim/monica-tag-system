...
class Contact extends VCardResource
{
    ...
    /**
     * Get the tags associated with the contact via the polymorphic pivot.
     *
    ...
    /**
     * Scope: return only contacts that have ALL of the specified tag IDs.
     *
     * Uses a single SQL statement with HAVING COUNT(DISTINCT tag_id) = N
     * so that no PHP-side looping is needed (as required by the brief).
     *
     * Example usage:
     *   Contact::withAllTags([1, 2])->paginate();
     *
     * @param  Builder  $query
    ...
    public function scopeWithAllTags(Builder $query, array $tagIds): Builder
    {
        ...
        return $query->whereIn('contacts.id', function ($sub) use ($tagIds, $count) {
            $sub->select('taggable_id')
                ->from('taggables')
                ->where('taggable_type', self::class)
                ->whereIn('tag_id', $tagIds)
                ->groupBy('taggable_id')
                ->havingRaw('COUNT(DISTINCT tag_id) = ?', [$count]);
        });
    }
